...
/**
 * AssetManager manages assets bundle configuration and loading.
 *
 ...
 *
 * You can modify its configuration by adding an array to your application customer-config under `components`
 ...
 * ```
 *
 * @property AssetConverterInterface $converter The assets converter. Note that the type of this property
...
{
    /**
     * @var array|boolean list of assets bundle configurations. This property is provided to customize assets bundles.
     ...
     *
     * The array keys are the assets bundle names, which typically are assets bundle class names without leading backslash.
     * The array values are the corresponding configurations. If a value is false, it means the corresponding assets
     * bundle is disabled and [[getBundle()]] should return null.
     *
     * If this property is false, it means the whole assets bundle feature is disabled and [[getBundle()]]
    ...
    public $bundles = [];
    /**
     * @var string the root directory storing the published assets files.
     */
    public $basePath = '@webroot/assets';
    /**
     * @var string the base URL through which the published assets files can be accessed.
     */
    public $baseUrl = '@web/assets';
    /**
     * @var array mapping from source assets files (keys) to target assets files (values).
     *
     * This property is provided to support fixing incorrect assets file paths in some assets bundles.
     * When an assets bundle is registered with a view, each relative assets file in its [[AssetBundle::css|css]]
     * and [[AssetBundle::js|js]] arrays will be examined against this map. If any of the keys is found
     * to be the last part of an assets file (which is prefixed with [[AssetBundle::sourcePath]] if available),
     * the corresponding value will replace the assets and be registered with the view.
     * For example, an assets file `my/path/to/jquery.js` matches a key `jquery.js`.
     *
     * Note that the target assets files should be absolute URLs, domain relative URLs (starting from '/') or paths
    ...
    public $assetMap = [];
    /**
     * @var boolean whether to use symbolic link to publish assets files. Defaults to false, meaning
     * assets files are copied to [[basePath]]. Using symbolic links has the benefit that the published
    ...
    /**
     * @var integer the permission to be set for newly published assets files.
     * This value will be used by PHP chmod() function. No umask will be applied.
     * If not set, the permission will be determined by the current environment.
     */
    public $fileMode;
    /**
     * @var integer the permission to be set for newly generated assets directories.
     * This value will be used by PHP chmod() function. No umask will be applied.
    ...
    /**
     * @var boolean whether to append a timestamp to the URL of every published assets. When this is true,
     * the URL of a published assets may look like `/path/to/assets?v=timestamp`, where `timestamp` is the
     * last modification time of the published assets file.
    ...
    /**
     * @var callable a callback that will be called to produce hash for assets directory generation.
     ...
     * ```
     *
     * where `$path` is the assets path. Note that the `$path` can be either directory where the assets
     ...
     * the relative assets files in the copying.
     *
     * If this is not set, the assets manager will use the default CRC32 and filemtime in the `hash`
     * method.
     *
    ...
    /**
     * Returns the named assets bundle.
     *
     * This method will first look for the bundle in [[bundles]]. If not found,
     * it will treat `$name` as the class of the assets bundle and create a new instance of it.
     *
     * @param string $name the class name of the assets bundle (without the leading backslash)
     * @param boolean $publish whether to publish the assets files in the assets bundle before it is returned.
     * If you set this false, you must manually call `AssetBundle::publish()` to publish the assets files.
     * @return AssetBundle the assets bundle instance
     * @throws InvalidConfigException if $name does not refer to a valid assets bundle
     */
    public function getBundle($name, $publish = true)
    {
        if ($this->bundles === false) {
            ...
            throw new InvalidConfigException("Invalid assets bundle configuration: $name");
        }
    }

    /**
     * Loads assets bundle class by name
     *
    ...
    /**
     * Returns the actual URL for the specified assets.
     * The actual URL is obtained by prepending either [[baseUrl]] or [[AssetManager::baseUrl]] to the given assets path.
     * @param AssetBundle $bundle the assets bundle which the assets file belongs to
     * @param string $asset the assets path. This should be one of the assets listed in [[js]] or [[css]].
     * @return string the actual URL for the specified assets.
     */
    ...
    /**
     * Returns the actual file path for the specified assets.
     * @param AssetBundle $bundle the assets bundle which the assets file belongs to
     * @param string $asset the assets path. This should be one of the assets listed in [[js]] or [[css]].
     * @return string|boolean the actual file path, or false if the assets is specified as an absolute URL
     */
    ...
    /**
     * Returns the assets converter.
     * @return AssetConverterInterface the assets converter.
     */
    ...
    /**
     * Sets the assets converter.
     * @param array|AssetConverterInterface $value the assets converter. This can be either
     * an object implementing the [[AssetConverterInterface]], or a configuration
     * array that can be used to create the assets converter object.
     */
     ...
     *
     * If the assets is a file, its file modification time will be checked to avoid
     * unnecessary file copying.
     *
     * If the assets is a directory, all files and subdirectories under it will be published recursively.
     ...
     *
     * @param string $path the assets (file or directory) to be published
     ...
     *
     * @return array the path (directory or file path) and the URL that the assets is published as.
     * @throws InvalidParamException if the assets to be published does not exist.
     */
    ...
    /**
     * Publishes a file.
     * @param string $src the assets file to be published
     * @return array the path and the URL that the assets is published as.
     * @throws InvalidParamException if the assets to be published does not exist.
     */
     ...
     * @param string $src the assets directory to be published
     * @param array $options the options to be applied when publishing a directory.
     * The following options are supported:
     *
     * - only: array, list of patterns that the file paths should match if they want to be copied.
     ...
     *   This overrides [[forceCopy]] if set.
     *
     * @return array the path directory and the URL that the assets is published as.
     * @throws InvalidParamException if the assets to be published does not exist.
     */
