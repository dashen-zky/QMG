...
function phorum_htmlpurifier_save_settings()
{
    ...
    if (phorum_htmlpurifier_config_file_exists()) {
        echo "Cannot update settings, <code>mods/htmlpurifier/customer-config.php</code> already exists. To change
        settings, edit that file. To use the web form, delete that file.<br />";
    } else {
        $config = phorum_htmlpurifier_get_config(true);
        if (!isset($_POST['reset'])) $config->mergeArrayFromForm($_POST, 'customer-config', $PHORUM['mod_htmlpurifier']['directives']);
        $PHORUM['mod_htmlpurifier']['customer-config'] = $config->getAll();
    }
