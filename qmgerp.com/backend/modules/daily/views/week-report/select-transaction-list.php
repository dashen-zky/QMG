...
?>
<?= $this->render('transaction-list-filter-form',[
    ...
    <?php foreach ($transactionList['list'] as $item) :?>
        <tr>
            <td>
                <input type="checkbox"
                       <?= (!empty($ser_filter)
                           && strpos($ser_filter, $item['uuid'])) ? 'checked':''?>
                       class="transaction-uuid"
                       name="uuid"
                       id="<?= $item['uuid'] . 'and' . $item['status']?>"
                       value="<?= $item['uuid']  . 'and' . $item['status']?>">
            </td>
            <td><a href="#" class="show transaction-title" url="<?= Url::to([
