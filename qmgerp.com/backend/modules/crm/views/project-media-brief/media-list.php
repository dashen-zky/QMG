...
<table class="table">
    ...
    <?php foreach ($briefList['list'] as $item) :?>
        <tr>
            ...
            <td>
                <a url="<?= Url::to([
                    '/crm/project-media-brief/show-in-media-entrance',
                    'uuid'=>$item['uuid'],
                    'edit'=>($item['status'] == ProjectMediaBriefConfig::StatusAssessed),
                ])?>" class="show" href="#">
                    <?= $item['title']?>
                </a>
            </td>
            ...
            <td>
                <?php if(ProjectMediaBrief::canAssess($item['status'])) :?>
                    <a href="<?= Url::to([
                        '/crm/project-media-brief/assess-succeed',
                        'uuid'=>$item['uuid'],
                    ])?>">通过</a>
...
$Js = <<<Js
$(function() {
    $('.meida-brief-list').on('click','.assess-refused', function() {
        var panel = $(this).parents('.meida-brief-list');
