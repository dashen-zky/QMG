...
<?php $form = ActiveForm::begin([
    ...
    'method' => 'post',
    'action' => ['/crm/project-config/update'],
    'fieldConfig' => [
    ...
    <tbody>
        ...
        <?php foreach($config[$key] as $index => $value) :?>
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-xs btn-primary removeRow"><i class="fa fa-2x fa-minus"></i></button>
                </td>
            </tr>
        <?php endforeach?>
        <?php endif?>
        <tr><td></td><td></td>
            <td>
                <button type="button" id="<?= $editKey?>" class="btn btn-xs btn-primary addRow" name="<?= $key?>">
