    ...
    <table class="table table-striped">
        ...
        <?php foreach ($projectList['projectList'] as $item) :?>
            <tr>
                ...
                <td><?= $item['apply_done_name']?></td>
                <td><?= $item['done_time'] != 0 ? date('Y-m-d', $item['done_time']):null ?></td>
                <td>
                    <?php if($item['status'] == ProjectConfig::StatusDoneApplying) :?>
                    <a href="<?= Url::to([
                        '/crm/project/done-assess-passed',
                        'uuid'=>$item['uuid']
                    ])?>">通过</a>
