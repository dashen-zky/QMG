...
<table class="table">
    ...
    <tr>
        ...
        <td>
            ...
            $uuids = $employee->getOrdinateUuids(RBACManager::ProjectModule);
            ...
            ?>
            <?= Html::dropDownList('AnniversaryAchievementForm[manager_uuid]',
                isset($formData['manager_uuid'])?$formData['manager_uuid']:'',
                ViewHelper::appendElementOnDropDownList($employeeList),[
                    'class'=>'form-control manager-uuid',
...
<span class="col-md-12">
    ...
    <span class="col-md-4 displayBlockWhileEdit" style="display: <?= isset($show)&&$show?'none':'block'?>">
        <input url="<?= Url::to([
            '/statistic/project-statistic/validate-anniversary-achievement'
...
<?= $this->render('@webroot/../views/site/error-modal',[
    'message'=>'亲，验证失败，请检查一下这个项目经理的该年度目标是否已经制定好！！'
])?>
</div>
<?php
$JS = <<<JS
$('.AnniversaryAchievementForm').on('click', '.validate-submit', function() {
    ...
    var url = self.attr('url') + '&manager_uuid=' + 
    form.find('.manager-uuid').val() + '&year=' + form.find('.year').val();
