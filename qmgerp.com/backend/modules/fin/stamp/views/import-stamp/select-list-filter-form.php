...
<input hidden name="ListFilterForm[entrance]" value="<?= isset($entrance)? $entrance:false?>">
<input class='stamp-name' disabled hidden
       value="<?= isset($filters['stamp_uuid'])?$filters['stamp_name']:''?>">
<input class='stamp-uuid' name="ListFilterForm[stamp_uuid]" hidden
       value="<?= isset($filters['stamp_name'])?$filters['stamp_uuid']:''?>">
<table class="table">
    <tbody>
    <tr>
    ...
    <tr>
        ...
        <td>
            <div class="col-md-12">
                    <span class="col-md-6" style="padding: 0px">
                    <?= Html::textInput('ListFilterForm[min_checked_time]',
                        isset($formData['min_checked_time'])?$formData['min_checked_time']:'',
                        [
                            'class' => 'input-section datetimepicker form-control col-md-12',
                            'placeholder'=>'开始日期'
                        ]) ?>
                    ...
                    <span class="col-md-6" style="padding: 0px">
                        <?= Html::textInput('ListFilterForm[max_checked_time]',
                            isset($formData['max_checked_time'])?$formData['max_checked_time']:'',
                            [
                                'class' => 'input-section datetimepicker form-control col-md-12',
                                'placeholder'=>'截止日期'
                            ]) ?>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                panel.find('.pagination').on('click', 'li', function() {
                    var stamp_modal = $(this).parents('.modal');
