    ...
    <div class="tab-content">
        <div class="tab-pane fade <?= (!isset($tab) || $tab === 'interview-list')?'active in':''?>" id="interview-list">
            <?= $this->render('list-panel',[
                'candidateList'=>isset($candidateList)?$candidateList:$candidate->assessCandidateList(),
                'ser_filter'=>isset($ser_filter)?$ser_filter:'',
                'list_filter_action'=>['/recruitment/interview/assess-list-filter'],
                'list_file'=>'assess-list',
            ])?>
        </div>
    </div>
</div>