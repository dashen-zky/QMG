 ...
 */
	...
	$.jstree.defaults.core = {
		 ...
		 *
		 * It is possible to pass in a standard jQuery-like AJAX customer-config and jstree will automatically determine if the response is JSON or HTML and use that to populate the tree.
	...
	$.jstree.core.prototype = {
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chc === false || ($.isFunction(chc) && chc.call(this, chk, obj, par, pos, more) === false) || (chc && chc[chk] === false)) {
				this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_03', 'reason' : 'User customer-config for core.check_callback prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				return false;
			}
			return true;
		},
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		touch : true,
		/**
		 * controls whether items can be dropped anywhere on the node, not just on the anchor, by default only the node anchor is a valid drop target. Works best with the wholerow plugin. If enabled on mobile depending on the interfaces it might be hard for the user to cancel the drop, since the whole tree container will be a valid drop target.
		 * @name $.jstree.defaults.dnd.large_drop_target
		 * @plugin dnd
		 */
		large_drop_target : false,
		/**
		 * controls whether a drag can be initiated from any part of the node and not just the text/icon part, works best with the wholerow plugin. Keep in mind it can cause problems with tree scrolling on mobile depending on the interfaces - in that case set the touch option to "selected".
 ...
 */
	 ...
	 *
	 * It is possible to set this to a standard jQuery-like AJAX customer-config.
 ...
 */
	...
	$.jstree.defaults.search = {
		/**
		 * a jQuery-like AJAX customer-config, which jstree uses if a server should be queried for results.
		 * 
